...
print('Starting accession report in ' + 'https://docs.google.com/spreadsheets/d/' + str(the_sheet_id) + '/edit?usp=sharing')
...
if mode=='Prod':
    ...
    with open(avery_acc_file, "w+") as f:
        try:
            ...
            raise ValueError("There was an error in getting Avery accession data!")
    ...
    with open(rbml_acc_file, "w+") as f:
        try:
            ...
            raise ValueError("There was an error in getting RBML accession data!")
...
for f in the_info:
    ...
    for an_accession in the_data:
        ...
        cm_dict = {"processing_priority":"processing_priority", "processing_status":"processing_status"}
        ...
        # Fiscal year
        if the_date.year < 1700: acc_info["fiscal-year"] = ""
        else: 
            if the_date.month > 6:
    ...
    print('Processed ' + str(len(all_rows)) + ' records in ' + repo_name + '.' )
    ...
    for i in the_output:
        i_date = dateutil.parser.isoparse(i[18]).date() # i[18] = the create date column

        if i_date > begin_of_week:
            # print('NOOB! ' + str(i[0]) + ' ' + str(i[18])) # Was created this week
    ...
    if len(the_recents[repo_name]) > 0: 
        print(' ')
        print(str(len(the_recents[repo_name])) + ' accessions recently added in ' + repo_name + ': ')
        print('-----------')
        ...
        print(' ')
        print('No recently created accessions in ' + repo_name)
...
if mode == 'Test':
    the_log = '[TEST] Data imported from ' + target_server + ' by ' + my_name + '. Start: ' + start_time  +  '. Finished: ' + end_time + ' (duration: ' + my_duration + ').'
else:
    the_log = 'Data imported from ' + target_server + ' by ' + my_name + '. Start: ' + start_time  +  '. Finished: ' + end_time + ' (duration: ' + my_duration + ').'
...
print('Script done. Updated data is available at ' + 'https://docs.google.com/spreadsheets/d/' + str(the_sheet_id) + '/edit?usp=sharing')
