...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:marc="http://www.loc.gov/MARC21/slim" exclude-result-prefixes="xs marc" version="2.0">
	...
	<xsl:template match="/">
		...
		<xsl:message>
			...
			<xsl:value-of select="count( repository/record[contains(header/identifier, '/resources/')])"/>
			<xsl:value-of select="$lf"/>
		</xsl:message>
		<collection>
	...
	<xsl:template match="marc:record">
		...
		<xsl:variable name="repo">
			<xsl:value-of select="replace(ancestor::record/header/identifier, '^.*?repositories/(\d)/.*?$','$1')"/>
		</xsl:variable>
		<record>
			<xsl:variable name="dataFields">
				...
				<!-- 965 added to all records to exclude them from Voyager authority control-->
			...
			<xsl:choose>
				...
				<xsl:when test="contains(ancestor::record/header/identifier, 'repositories/6')">
					<controlfield tag="008">
						...
						<xsl:choose>
							<xsl:when test="marc:datafield[@tag='245']/marc:subfield[@code='g']                                 and                                  not(marc:datafield[@tag='245']/marc:subfield[@code='f'])">
								<xsl:value-of select="replace(substring(marc:controlfield[@tag='008'], 1, 7), 'i', 'k')"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="substring(marc:controlfield[@tag='008'],1,7)"/>
							</xsl:otherwise>
						</xsl:choose>
   ...
   -->
	...
	<xsl:template match="marc:datafield[@tag = '041'][marc:subfield[not(normalize-space(text()))]][../not(marc:datafield[@tag = '546'])]">
		<!-- Do nothing -->
	</xsl:template>
	<!-- RBMLBOOKS -->
