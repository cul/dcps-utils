...
--> 
    <xsl:param name="filename">/Users/dwh2128/Documents/ACFA/OAI_local/20200330/20200330.asClean.xml</xsl:param>
...
<!-- The part that goes to the updated file list for the Rails app. -->
            ...
            <xsl:for-each select="collection/record">
                ...
                <xsl:message>Title: <xsl:value-of select="datafield[@tag='245']/subfield[@code='a']"/> </xsl:message>
 <!--               
                <xsl:message>Identifier: <xsl:value-of select="header/identifier"/></xsl:message>
                <xsl:message>Datestamp: <xsl:value-of select="header/datestamp"/></xsl:message>
                
 -->              
                <xsl:message>BibID: <xsl:value-of select="datafield[@tag='099']/subfield[@code='a']"/></xsl:message> 
                <xsl:if test="normalize-space(datafield[@tag='856']/subfield[@code='u'])">
                <xsl:message>Finding aid: <xsl:value-of select="datafield[@tag='856']/subfield[@code='u']"/> </xsl:message>
                </xsl:if>
